...
import { BrainCircuit, FileText, ShieldCheck, MessageSquareText } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="w-[80px] lg:w-64 bg-[#0f172a] text-slate-300 flex flex-col h-full shadow-2xl no-print transition-all duration-300 z-50">
      {/* Brand */}
      <div className="h-24 flex items-center justify-center lg:justify-start lg:px-6 border-b border-slate-800/50">
      ...
      <div className="flex-1 py-8 px-3 space-y-4">
        <div className="text-xs font-bold text-slate-500 uppercase px-4 mb-2 hidden lg:block">Core Modules</div>
        {modules.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="hidden lg:block text-left">
                ...
                <span className={`text-xs block mt-0.5 ${isActive ? 'text-indigo-200' : 'text-slate-500'}`}>{item.desc}</span>
              </div>
      ...
      <div className="p-6 border-t border-slate-800/50 text-center lg:text-left">
        <div className="text-xs text-slate-600">
          ...
          <p className="hidden lg:block">Version 2.0.1</p>
        </div>
      </div>
    </div>
  );
